...
const model_n = 12,
    frame_per_model = 250;
let progress = 0;
let first_loaded = new Array(model_n);

for (var i = 0; i < model_n; i++) {
    first_loaded[i] = false;
}
...
function videoloaded(model_n) {
    progress += 1;
    console.log(progress);
    document.body.style.overflow = 'visible';
    $('.container').css('visibility', 'visible');
    $('.spinner-grow').css('display', 'none');
    console.log(`dd ${model_n}`);
    if (first_loaded[model_n] == false) {
        document.getElementById(`video${model_n+1}`).currentTime = frame_per_model * i * time_per_frame;
        first_loaded[model_n] = true;
    }
    document.getElementById(`video${model_n+1}`).pause();
}
...
for (var i = 0; i < model_n; i++) {
    document.body.innerHTML +=
        `<div id="scene${i+1}" class="container" style="visibility:hidden">
            <video id="video${i+1}" class="model" playsinline autoplay muted preload="auto">
            <source src="${video_mp4_url}" type="video/mp4">
            </video>
        </div>`;
    document.getElementById(`video${i+1}`).addEventListener('canplaythrough', videoloaded(i));
}
